...
<body>
    ...
    <!-- Edit Job -->
     <?php $getJobPostByID = getJobPostByID($pdo, $_GET['post_id']);?>
    <div class="container">
        <h2 class="create-job">Edit Job</h2>
        <div class="job-container">
           <form class="job-form" action="../core/handleForms.php?post_id=<?php echo $_GET['post_id']?>" method="POST">
               <p>
                   <label class="job-label" for="job-title">Job Title:</label> <br>
                   <input class="job-details" type="text" name="job_title" required value="<?php echo $getJobPostByID['job_title']; ?>">
               </p>
               <p>
                    <label class="job-label" for="job-description">Job Description:</label> <br>
                    <textarea class="job-details" name="job_description" rows="5" cols="50" required><?php echo $getJobPostByID['job_description']; ?></textarea>
                </p>
               <p>
                   <input class="submit-btn job-btn" class="submit-form" type="submit" name="editJobBtn" value="Edit">
               </p>
           </form>
        </div>
     </div>
